...
def process_packet(packet):
    if packet.haslayer(IP) and packet[IP].src == CLIENTE_IP and packet[IP].dst == SERVIDOR_IP and packet.haslayer(Raw):
        ...
        if b'SELECT' in payload_original:
            fuzz = b" " + bytes(RandString(20))
            print(f"[!] ATAQUE: Haciendo Fuzzing, inyectando basura: {fuzz.decode(errors='ignore')}")
            packet[Raw].load += fuzz
...
print("[*] ATAQUE 'FUZZING ALEATORIO' ACTIVO. Esperando consultas SELECT...")
try:
